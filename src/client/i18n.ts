...
const ZH = {
  ...
  divergedAhead: (n: number, ref: string) => `本地领先 ${ref} ${n} 个提交：`,
  divergedMore: (n: number) => `…另有 ${n} 个提交未列出`,
  realignWhat: (branch: string, ref: string) =>
    `「备份并对齐远端」会先把当前 HEAD 存成分支 ${branch}（提交一个都不丢，随时能切回去），再把工作副本硬重置到 ${ref}，然后照常装依赖、重建前端。`,
  realignDirty: '工作区有未提交改动——重置会抹掉它们。先自行处理 git status 再来对齐。',
  realignBtn: '备份本地提交并对齐远端',
  backupSaved: (branch: string) => `本地提交已备份到分支 ${branch}。`,
  checking: '正在检查更新…',
  upToDate: '已是最新版本',
...
const EN: typeof ZH = {
  ...
  divergedMore: (n: number) => `…and ${n} more not listed`,
  realignWhat: (branch: string, ref: string) =>
    `“Back up and align to remote” saves the current HEAD as branch ${branch} (no commit is lost — you can switch back any time), hard-resets the working copy to ${ref}, then installs deps and rebuilds as usual.`,
  ...
  backupSaved: (branch: string) => `Local commits are backed up on branch ${branch}.`,
